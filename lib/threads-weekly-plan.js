...
function nextMondayJst(now = new Date()) {
  const jst = new Date(now.toLocaleString('en-US', { timeZone: 'Asia/Tokyo' }));
  const day = jst.getDay();
  const daysUntilMonday = day === 1 ? 0 : (8 - day) % 7;
  jst.setDate(jst.getDate() + daysUntilMonday);
...
function topicCopy(theme) {
  const map = {
    care: {
      label: 'うるおいケア週間',
      opener: '肌や髪の乾燥って、気づいた時にはもうだいぶしんどい。',
      worry: 'ちゃんとケアしなきゃと思うほど、手順が重くなる日がある。',
      question: '顔より先に、手とか髪のパサつきが気になる人いませんか。',
...
function buildWeeklyThreadsPlan(products = [], startDate = nextMondayJst()) {
