...
function buildWeeklyThreadsPlan(products = [], startDate = new Date()) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    rows.push({
      ...
      url: product.url || '',
      text: appendUrl(text, product.url),
      status: '未投稿',
    });
  }
