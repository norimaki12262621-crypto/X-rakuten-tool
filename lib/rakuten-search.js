function buildSearchParams({ keyword = '', minPrice = 1, maxPrice = 999999, sort = '-reviewCount', hits = 30, page = 1, itemCode = '' } = {}, { includeAccessKey = true } = {}) {
  let searchKey;
  if (itemCode) {
    searchKey = { itemCode };
  } else {
...
async function searchRakuten(options = {}) {
  ...
  try {
    ...
    const legacyParams = buildSearchParams(options, { includeAccessKey: false });
